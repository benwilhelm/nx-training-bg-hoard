...
export default async function (host: Tree) {
  ...
  updateJson(host, 'tools/generators/tagged-lib/schema.json', (schemaJson) => {
  ...
  const content = host.read('tools/generators/tagged-lib/index.ts', 'utf-8');
  const newContent = replaceScopes(content, scopes);
  host.write('tools/generators/tagged-lib/index.ts', newContent);
